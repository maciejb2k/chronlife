class DiseaseSymptomsController < BaseController
  ...
  before_action :load_predefined_symptoms, only: %i[new edit create]
  ...
  def create
    @disease_symptom = @disease.disease_symptoms.new(disease_symptom_params)
  ...
  def update
    respond_to do |format|
      if @disease_symptom.update(disease_symptom_params)
        format.html do
          redirect_to disease_symptom_url(@disease_symptom),
  ...
  def destroy
    ...
    respond_to do |format|
      format.html do
        redirect_to disease_symptoms_url, notice: "Disease symptom was successfully destroyed."
      end
  ...
  def set_disease_symptom
    @disease_symptom = @disease.disease_symptoms.find(params[:id])
  end
  ...
  def set_breadcrumbs_show
    add_breadcrumb("pokaż objaw", [@disease, @disease_symptom])
  end
